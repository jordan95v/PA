<li>
    <a class="dropdown-item" href="" data-bs-toggle="modal" data-bs-target="#film">Ajouter un film</a>
</li>
<li>
    <a class="dropdown-item" href="" data-bs-toggle="modal" data-bs-target="#event">Évènement</a>
</li>
<li>
    <a class="dropdown-item" href="" data-bs-toggle="modal" data-bs-target="#newsletter">Newsletter</a>
</li>
<li>
    <a class="dropdown-item" href="" data-bs-toggle="modal" data-bs-target="#users">Liste des users</a>
</li>